...
class Profile(models.Model):
    ...
    def __str__(self):
        return f"Profile(user={self.user.username}, weight_system={self.weight_system}, weight={self.weight})"
...
class Exercise(models.Model):
    ...
    @classmethod
    def body_part_list(cls):
        return [item[0] for item in cls.BODY_PARTS]

    @classmethod
    def equipment_list(cls):
        return [item[0] for item in cls.EQUIPMENT]
...
class WorkoutExerciseDetail(models.Model):
    workout_exercise = models.ForeignKey(WorkoutExercise, related_name="workout_exercise_details", on_delete=models.CASCADE)
    sets = models.PositiveIntegerField()
    reps = models.PositiveIntegerField()
    weight = models.PositiveIntegerField()

    def __str__(self):
        return f"WorkoutExerciseDetail(workout_exercise={self.workout_exercise.exercise.name}, sets={self.sets}, reps={self.reps}, weight={self.weight})"
